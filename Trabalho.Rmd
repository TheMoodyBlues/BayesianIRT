...
Como os resultados utilizando o modelo do jags não foi muito satisfatório abaixo apresentamos os traços das cadeias para ambos os parâmetros dos dois primeiros itens:

```{r}
bayesplot::mcmc_trace(fit.jags, pars = c("alpha[1]","beta[1]","alpha[2]","beta[2]"))
```

Vemos que as cadeias dos parâmetros de dificuldade convergiram, porém os parâmetros de discriminação não apresentam cadeias identicas em todos os casos, vale notar que as cadeias dos betas são a mesma porém com o sinal invertido, abaixo vemos que alguns dos thetas também apresentam esse comportamento, o que faz sentido com a parametrização do modelo pois são os termos que são multiplicados. 

```{r}
bayesplot::mcmc_trace(fit.jags, pars = c("theta[1]","theta[2]","theta[3]","theta[4]"))
```

Esse comportamento é, em parte, devido às prioris selecionadas, para um resultado mais satisfatório seria necessário utilizar outras distribuições que assumissem apenas valores positivos ou restringir os valores de beta.
