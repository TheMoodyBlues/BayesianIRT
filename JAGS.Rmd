...
cat("model{
    ...
    for(n in 1:N){
      resp[n] ~ dbern(z[n])
      logit(z[n]) <- -alpha[item[n]] + beta[item[n]]*theta[id[n]] + erro[n]
    }
...
coefs.jags <- sum.jags$statistics

coefs.jags[grepl("alpha", rownames(coefs.jags)),]
coefs.jags[grepl("beta", rownames(coefs.jags)),]
